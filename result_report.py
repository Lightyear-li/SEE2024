...
def analysis(mark):
    ...
    for meal in to_preprocess:
        content = pd.read_csv(f'./data/report/{meal}_{prob}_{mark}.csv',index_col=False)
    ...
    AAS = round((report.loc[report['id']=='brk','AAS'].values[0] * report.loc[report['id']=='brk','Protein_g'].values[0] + report.loc[report['id']=='lun','AAS'].values[0] * report.loc[report['id']=='lun','Protein_g'].values[0] + report.loc[report['id']=='din','AAS'].values[0] * report.loc[report['id']=='din','Protein_g'].values[0]) / report.loc[report['id']=='total','Protein_g'].values[0], 2)
    ...
    for meal in to_preprocess:
        diet = pd.read_csv(f'./data/report/{meal}_{prob}_{mark}.csv',index_col=False)
    ...
    report.to_csv(f'./data/report/{prob}_{mark}_report.csv',index=False)